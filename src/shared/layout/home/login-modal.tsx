...
import { Modal, Form, ModalHeader, ModalBody, Alert, FormGroup, Label, Input, FormFeedback, ModalFooter, Button } from 'reactstrap';
...
const LoginModal: React.FC<ILoginModalProps> = props => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('actool-authenticationToken') !== null || localStorage.getItem('actool-authenticationToken') !== null) {
      props.history.push('/');
    }
  ...
  return (
    <Modal isOpen toggle={handleClose} backdrop id="login-modal" autoFocus={false}>
      <Form id="login-form" onSubmit={handleSubmit}>
        ...
        <ModalFooter>
          <Button type="button" color="secondary" onClick={handleClose} tabIndex={1} disabled={props.loginSuccess}>
            Annulla
          </Button>{' '}
          <Button color="primary" type="button" onClick={handleSubmit} disabled={props.loginSuccess} >
            Login
          </Button>
        </ModalFooter>
      </Form>
